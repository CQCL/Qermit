...
def gen_randomised_circuit(circuit):
    ...
    randomisation_reg = randomised_circuit.add_c_register("randomisation", 8)

    rand_vals = [116, 154]

    for command_count, command in enumerate(circuit):

        # randomisation_reg = randomised_circuit.add_c_register(f"randomisation_{command_count}", 8)
        ...
        if command.op.type == OpType.ZZMax:
            ...
            # randomised_circuit.Phase(-1, condition_bits=[randomisation_reg[2], randomisation_reg[3]])
