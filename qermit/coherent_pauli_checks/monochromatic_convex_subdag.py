...
class MonochromaticConvexSubDAG():
    ...
    def _subdag_predecessors(self, subdag, node_subdag):
        ...
        return sum(
            [
                [
                    predecessor for predecessor in self.dag.predecessors(node)
    ...
    def _subdag_successors(self, subdag, node_subdag):
        ...
        return sum(
            [
                [
                    successor for successor in self.dag.successors(node)
    ...
    def _can_merge(self, subdag_one, subdag_two, node_subdag):
        ...
        subdag_two_pred = self._subdag_predecessors(subdag=subdag_two, node_subdag=node_subdag)
        for subdag_one_succ in self._subdag_successors(subdag=subdag_one, node_subdag=node_subdag):
            if any(descendant in subdag_two_pred for descendant in self.node_descendants[subdag_one_succ]):
                return False
        
        subgraph_one_pred = self._subdag_predecessors(subdag=subdag_one, node_subdag=node_subdag)
        for subdag_two_succ in self._subdag_successors(subdag=subdag_two, node_subdag=node_subdag):
            if any(descendant in subgraph_one_pred for descendant in self.node_descendants[subdag_two_succ]):
                return False
    ...
    def greedy_merge(self):
        ...
        node_subdag = {node: i for i, node in enumerate(self.node_coloured) if self.node_coloured[node]}
        ...
        while subgraph_merged:
            ...
            for subdag_one, subdag_two in permutations(node_subdag.values(), 2):
                ...
                if self._can_merge(
                    ...
                    for node in self._subdag_nodes(subdag=subdag_two, node_subdag=node_subdag):
